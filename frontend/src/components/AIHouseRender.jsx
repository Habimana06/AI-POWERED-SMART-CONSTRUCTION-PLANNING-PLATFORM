...
import { Sparkles, Loader2, AlertCircle, Download, CheckCircle2, Lock } from 'lucide-react';
...
export default function AIHouseRender({
  ...
  useEffect(() => {
    if (mode !== 'exterior' || !projectId || !designId || savedImageUrl || triggerRef.current) return;
    triggerRef.current = true;
    setGenerating(true);
    generateMutation.mutate();
  }, [mode, projectId, designId, savedImageUrl, renderKey]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 rounded-lg border border-steel-200 bg-steel-50 px-3 py-2 text-xs text-steel">
        ...
        <span>
          Full house image is generated automatically on the server from your saved floor plan and materials.
          Prompt and AI run in the backend — not shown here. Manual regenerate is disabled.
        </span>
      </div>
      ...
      <div className={`relative rounded-xl overflow-hidden border border-steel-100 bg-steel-50 min-h-[280px] max-h-[640px] ${aspectCls}`}>
        ...
        {!isLoading && !imageSrc && genError && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-4 text-center">
            ...
            <p className="text-xs text-steel">{genError}</p>
            <p className="text-[10px] text-concrete">Save the design again in Building Editor to retry server generation.</p>
          </div>
        )}
      </div>
    </div>
